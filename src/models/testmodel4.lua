...
local function createModel(opt)
    ...
    local function CFBlock(nChIn, nChOut, nChConf, nChFC, sz, input)
        ...
        local conv2 = cudnn.SpatialConvolution(nChOut,nChOut,sz,sz,1,1,(sz-1)/2,(sz-1)/2)(act2)
		-- add two more
        local bn3   = cudnn.SpatialBatchNormalization(nChOut)(conv2)
        local act3  = cudnn.ReLU(true)(bn3)
        local conv3 = cudnn.SpatialConvolution(nChOut,nChOut,sz,sz,1,1,(sz-1)/2,(sz-1)/2)(act3)
        local bn4   = cudnn.SpatialBatchNormalization(nChOut)(conv3)
        local act4  = cudnn.ReLU(true)(bn4)
        local conv4 = cudnn.SpatialConvolution(nChOut,nChOut,sz,sz,1,1,(sz-1)/2,(sz-1)/2)(act4)
        ...
        -- identity addition
        local id = nn.CAddTable()({identity, conv4})
    ...
    if opt.dataset == 'towncenter' then
        ...
        -- draw and save model
        graph.dot(model.fg, 'forward graph', './graphs/testmodel4')
   else
