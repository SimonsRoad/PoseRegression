...
local function createModel(opt)
    ...
    local function CFBlock(nChIn, nChOut, nChConf, nChFC, sz, input)
        ...
        --
		local bn_fc3   = cudnn.SpatialBatchNormalization(nChOut)(conv_fc2)
		local act_fc3  = cudnn.ReLU(true)(bn_fc3)
		local conv_fc3 = cudnn.SpatialConvolution(nChOut,nChOut,sz,sz,1,1,(sz-1)/2,(sz-1)/2)(act_fc3)

		-- identity shortcut
		local identity = shortcut(nChIn, nChOut, input)
		
		-- final output
		local output = nn.CAddTable()({identity, conv_fc3})
    ...
    if opt.dataset == 'towncenter' then
        ...
        local bn_end1   = cudnn.SpatialBatchNormalization(128)(cf2)
        ...
        local cat_last  = nn.JoinTable(2)({c1,c2, conv_end2})

        local bn_cat1   = cudnn.SpatialBatchNormalization(30*3)(cat_last)
        local act_cat1  = cudnn.ReLU(true)(bn_cat1)
        local conv_cat1 = cudnn.SpatialConvolution(30*3,256,1,1)(act_cat1)
        ...
        -- draw and save model
        graph.dot(model.fg, 'forward graph', './graphs/testmodel6')
   else
