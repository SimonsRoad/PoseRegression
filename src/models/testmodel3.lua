...
local function createModel(opt)
    ...
    if opt.dataset == 'towncenter' then
        ...
        local cf4, c4  = CFBlock(218,218,30,256,17,cf3) 	-- two more unit
        local cf5, c5  = CFBlock(248,248,30,256,17,cf4) 	

        local bn_end1   = cudnn.SpatialBatchNormalization(278)(cf5)
        local act_end1  = cudnn.ReLU(true)(bn_end1)
        local conv_end1 = cudnn.SpatialConvolution(278,256,1,1)(act_end1)
        ...
        local cat_last  = nn.JoinTable(2)({c1,c2,c3,c4,c5, conv_end2})

        local bn_cat1   = cudnn.SpatialBatchNormalization(30*6)(cat_last)
        local act_cat1  = cudnn.ReLU(true)(bn_cat1)
        local conv_cat1 = cudnn.SpatialConvolution(30*6,256,1,1)(act_cat1)
        ...
        -- draw and save model
        graph.dot(model.fg, 'forward graph', './graphs/testmodel3')
   else
