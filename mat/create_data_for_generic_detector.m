% create_data_for_generic_detector.m
...
%% img
imglist = sprintf('~/develop/PoseRegression/data/rendout/anc_y%03d_x%d/lists_tmp/img_pos.txt', y,x);
fid = fopen(imglist);
tline = fgetl(fid);
cnt = 0;
while ischar(tline)
    ...
    savefilename = [tmp(1:pos_a-1), sprintf('anc_y%03d_x%d/pos',y,x), tmp(pos_ped-7:end)];
    imwrite(im_res, savefilename);
    tline = fgetl(fid);
end
fclose(fid);
%% jsc
jsclist = sprintf('~/develop/PoseRegression/data/rendout/anc_y%03d_x%d/lists_tmp/jsc_pos.txt', y,x);
fid = fopen(jsclist);
tline = fgetl(fid);
cnt = 0;
while ischar(tline)
    ...
    pos_ped = strfind(tmp, '_Ped');
    savefilename = [tmp(1:pos_a-1), sprintf('anc_y%03d_x%d/jsc',y,x), tmp(pos_ped-7:end)];
